...
from a_star import a_star_algorithm, uniform_cost
...
def uniform_button_on_click():
    start = entry1.get()
    end = entry2.get()
    path = str(uniform_cost(start, end))
    result.config(text=path)
...
bfs_button = Button(window, text="uniform cost search", command=uniform_button_on_click)
bfs_button.pack()
